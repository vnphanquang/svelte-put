...
<p>
  <code>@svelte-put/avatar</code> provides several resolution strategies for image source that can be
...
<AvatarUsageSectionLayout>
  <p>
    ...
    than using a plain <code>img</code>, thus more helpful when used in conjunction with one/more
    solutions listed in the following sections.
  </p>
...
<AvatarUsageSectionLayout>
  ...
  <ol>
    <li>
      <code class="mt-0">Avatar</code> component with the <code class="mt-0">gravatar</code> prop:
...
<AvatarUsageSectionLayout>
  <p>
    As mentioned in
    <ResourceLink id={RESOLUTION_STRATEGY_SECTION_ID}>Resolution Strategies</ResourceLink>, multiple
    sources can be provided to form a chain that resolves to the first available image source.
  </p>
  <svelte:fragment slot="avatar">
    <AvatarMultiple />
  </svelte:fragment>
...
<AvatarUsageSectionLayout>
  <p>
    The <code>Avatar</code> component provides minimal global styles with 0 css specificity. See
    <ResourceLink
  ...
  <ul>
    ...
    <li>
      <code>--border-color</code> css variable bound to the <code>color</code> prop, made possible
      with <ResourceLink key="svelte --style-props" />.
    </li>
  </ul>
  <svelte:fragment slot="avatar">
    <AvatarCustomStyling />
