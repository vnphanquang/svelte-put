...
```bash title=npm
npm install --save-dev @svelte-put/preprocess-auto-slug@next
```

```bash title=pnpm
pnpm add -D @svelte-put/preprocess-auto-slug@next
```

```bash title=yarn
yarn add -D @svelte-put/preprocess-auto-slug@next
```
