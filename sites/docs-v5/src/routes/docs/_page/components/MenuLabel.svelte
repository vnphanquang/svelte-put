<script lang="ts">
	...
	let { align = 'left', class: cls, for: forHtml, children, ...rest }: HTMLLabelAttributes & { align?: 'left' | 'right' } = $props();

  const commonPathClasses = `transition-transform origin-left`;
</script>

<label
	class="cursor-pointer group flex items-center gap-2 text-xs hover:text-link {cls}"
