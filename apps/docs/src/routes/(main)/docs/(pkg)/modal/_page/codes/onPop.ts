...
let unsubscribe = store.onPop<FullCustomModal>(pushed.id, (resolved) => {
  /** */
});
...
function onPop(resolved: ModalResolved<FullCustomModal>) {
  /** */
}
...
const sideEffect: ModalResolveCallback<FullCustomModal> = (resolved) => {
  /** */
};
