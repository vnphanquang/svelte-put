...
```bash title=npm
npm install --save-dev @svelte-put/toc@next
```

```bash title=pnpm
pnpm add -D @svelte-put/toc@next
```

```bash title=yarn
yarn add -D @svelte-put/toc@next
```
...
The table of contents in this documentation site is generated by `toc` itself. Check out [its source code here](https://github.com/vnphanquang/svelte-put/blob/adb89085b72a09b43285ca392ca1b7552641d203/sites/docs/src/routes/(site)/docs/%2Blayout.svelte#L21-L29) (search for `toc`).
